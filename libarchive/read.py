...
from .ffi import (
    ...
    SKIP_CALLBACK, NO_OPEN_CB, NO_CLOSE_CB, page_size,
)
...
class ArchiveRead:
    ...
    def skip_data(self):
        """Discard the current entry's data without returning it.

        This still pulls and decodes the entry's bytes internally, so the
        decompressor's state advances exactly as if the data had been read.
        That property is what makes resumable streaming possible: to resume a
        forward-only stream at entry N you must feed the decoder everything
        before N, and this method does so without materializing files you have
        already extracted. See `libarchive.checkpoint`.
        """
        ffi.read_data_skip(self._pointer)
...
def stream_reader(
    ...
    passed to libarchive. Additionally, if the stream provides a `skip(request)`
    method, it is registered as libarchive's skip callback so the library can
    jump over unwanted data (e.g. already-extracted entries) without reading it
    — for a network-backed stream that means those bytes are never fetched.
    `skip(request)` must return the number of bytes actually skipped (0 if it
    can't skip, in which case libarchive falls back to reading and discarding).
    """
    ...
    def skip_func(archive_p, context, request):
    ...
    seek_cb = SEEK_CALLBACK(seek_func)
    skip_cb = SKIP_CALLBACK(skip_func)
    with new_archive_read(format_name, filter_name, passphrase) as archive_p:
        if stream.seekable():
            ffi.read_set_seek_callback(archive_p, seek_cb)
        if hasattr(stream, 'skip'):
            ffi.read_set_skip_callback(archive_p, skip_cb)
        ffi.read_open(archive_p, None, open_cb, read_cb, close_cb)
        yield ArchiveRead(archive_p, header_codec)
